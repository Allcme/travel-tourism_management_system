...
import java.sql.*; //for query execution
...
public class ForgetPassword extends JFrame implements ActionListener{
	...
	public void actionPerformed(ActionEvent ae) {
		if(ae.getSource() == search) {
			try {
				//Search and select where the username is present and store
				String query = "select * from account where username = '"+tfusername.getText()+"'";
				//Creating a object for database query
				Conn c = new Conn();
				
				//table se data fetch 
				ResultSet rs = c.s.executeQuery(query);
				//rs.next is a function where we can loop ResultSet function
				while(rs.next()) {
					//helps to get the value of given input
